...
class DatabaseSeeder {
    ...
    async clearData() {
        ...
        for (const table of tables) {
            await DB_1.default.from(table).delete();
        }
    }
    async seedUsers() {
        ...
        const users = [
            {
                ...
                password: await Authenticate_1.default.hash("password123"),
                role: "admin",
                is_admin: true,
                is_verified: true,
        ...
        await DB_1.default.table("users").insert(users);
        return users;
    }
    async seedClasses(users) {
        ...
        await DB_1.default.table("classes").insert(classes);
        return classes;
    }
        ...
        const studentClasses = [
            {
                ...
                enrollment_date: (0, dayjs_1.default)().subtract(3, 'months').format('YYYY-MM-DD'),
                is_active: true,
                created_at: (0, dayjs_1.default)().valueOf()
            },
            {
            ...
            },
            {
                ...
                enrollment_date: (0, dayjs_1.default)().subtract(2, 'months').format('YYYY-MM-DD'),
                is_active: true,
                created_at: (0, dayjs_1.default)().valueOf()
            }
        ];
        await DB_1.default.table("student_classes").insert(studentClasses);
    }
    async seedParentStudentRelations(users) {
        ...
        await DB_1.default.table("parent_student_relations").insert(relations);
    }
    async seedAttendanceSessions(classes, users) {
        ...
        await DB_1.default.table("attendance_sessions").insert(sessions);
        return sessions;
    }
    async seedAttendanceRecords(users, classes) {
        console.log("✅ Seeding attendance records...");
        const students = users.filter(u => u.role === 'student');
        const sessions = await DB_1.default.from("attendance_sessions").select("*");
        const records = [];
        for (const session of sessions) {
            const classStudents = await DB_1.default.from("student_classes")
            ...
            for (const student of classStudents) {
                ...
                records.push({
                    ...
                    status: status,
                    scanned_at: status !== 'absent' ? (0, dayjs_1.default)(session.starts_at).add(Math.random() * 15, 'minutes').toDate() : null,
        ...
        await DB_1.default.table("attendance_records").insert(records);
    }
    async seedTeacherJournals(users, classes) {
        ...
        for (let i = 0; i < 20; i++) {
            ...
            journals.push({
                ...
                journal_date: (0, dayjs_1.default)().subtract(Math.floor(Math.random() * 30), 'days').format('YYYY-MM-DD'),
        ...
        await DB_1.default.table("teacher_journals").insert(journals);
    }
    async seedExams(users, classes) {
        ...
        for (const teacher of teachers) {
            ...
            for (const cls of teacherClasses) {
                for (let i = 0; i < 2 + Math.floor(Math.random() * 2); i++) {
                    ...
                    const startTime = (0, dayjs_1.default)().add(Math.floor(Math.random() * 30) + 1, 'days').hour(8 + i * 2).minute(0);
                    exams.push({
                        id: examId,
                        teacher_id: teacher.id,
                        class_id: cls.id,
        ...
        await DB_1.default.table("exams").insert(exams);
        await DB_1.default.table("exam_questions").insert(examQuestions);
    }
}
