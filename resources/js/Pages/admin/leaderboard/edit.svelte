<script>
   ...
   function submitForm() {
      ...
      isSubmitting = true;
      router.put(`/admin/leaderboard/${entry.id}`, form, {
         preserveState: false,
         onSuccess: () => {
            toast.success('Entry leaderboard berhasil diupdate', {
               duration: 3000,
               position: 'top-right'
            });
         },
         onError: (errors) => {
            const errorMsg = errors?.error || Object.values(errors)[0] || 'Gagal mengupdate data';
...
<svelte:head>
   <title>Edit Leaderboard - NETSA Admin</title>
</svelte:head>
   ...
   <main class="max-w-4xl mx-auto py-8 sm:px-6 lg:px-8">
      <div class="px-4 py-6 sm:px-0">
         <div class="mb-8 fade-in-up">
            <h2 class="text-3xl font-bold text-gray-900 mb-2">Edit Entry Leaderboard</h2>
            <p class="text-gray-600">Edit data kelas di leaderboard</p>
         </div>
